...
<html lang="en">
	<head>
		...
		<title>编辑${(devEntity.classNameDesc)!'-'} - Powered By APP TEAM</title>
				...
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" >
						...
						<ul class="breadcrumb">
							...
							<li class="active">
								<span>编辑${(devEntity.classNameDesc)!'-'}</span>
							</li>
					...
					<div class="page-content">
						...
						<!-- 表单开始 -->
						<form id="inputForm" action="update.jhtml" class="form-horizontal" role="form" method="post" <#if devEntity.hasAnyFileTypeAttribute() >enctype="multipart/form-data"</#if> >
							<!-- 实体ID隐藏域 -->
							<input type="hidden" name="id" value="${r'$'}{${(devEntity.unCapitalizedClassName)!'-'}.id}" />
							<div class="row">
								<div class="col-xs-12">
									<div class="tabbable">
									    ...
									    <div class="tab-content">
											...
											<#if devEntity.isTreeEntity() >
														...
														<div class="clearfix">
															<select id="parentId" name="parentId" class="col-xs-10 col-sm-5" >
																...
																[#list ${(devEntity.unCapitalizedClassName)!'-'}Tree as ${(devEntity.unCapitalizedClassName)!'-'}Tmp ]
																	[#if ${(devEntity.unCapitalizedClassName)!'-'}Tmp != ${(devEntity.unCapitalizedClassName)!'-'} && !children?seq_contains(${(devEntity.unCapitalizedClassName)!'-'}Tmp)]
																	<option value="${r'$'}{${(devEntity.unCapitalizedClassName)!'-'}Tmp.id}" [#if ${(devEntity.unCapitalizedClassName)!'-'}Tmp == ${(devEntity.unCapitalizedClassName)!'-'}.parent ]selected="selected"[/#if]>
												...
												<div class="form-group">
													...
													<div class="col-sm-9">
														<div class="clearfix">
															<input type="text" id="order" name="order" value="${r'$'}{(${(devEntity.unCapitalizedClassName)!'-'}.order)!}" class="col-xs-10 col-sm-5" />
														</div>
													</div>
												</div>
											</#if>
												...
												<#else>
													...
													<#if attribute.isAttributeTypeBeBoolean() >
														<div class="clearfix">	
															<div class="checkbox">
																<label>
															        <input id="${(attribute.unCapitalizedName)!'-'}" name="${(attribute.unCapitalizedName)!'-'}" value="true" [#if (${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'})!]checked="checked"[/#if] class="ace ace-switch ace-switch-2" type="checkbox" />
													...
													<#elseif attribute.isAttributeTypeBeDate() >
														<div class="clearfix">
															<input type="text" id="${(attribute.unCapitalizedName)!'-'}" name="${(attribute.unCapitalizedName)!'-'}" value="${r'$'}{(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'})!}" class="Wdate col-xs-10 col-sm-5" onfocus="WdatePicker()" maxlength="100" />			
														</div>
													<#-- 枚举类型 属性处理 -->
													<#elseif attribute.isAttributeTypeBeEnum() >
														<div class="clearfix">
															<select name="${(attribute.unCapitalizedName)!'-'}" class="col-xs-10 col-sm-5" >
																...
																<option value="${r'$'}{${attribute.capitalizedName}}" [#if (${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'})! == ${attribute.capitalizedName}]selected="selected"[/#if] >${r'$'}{(${attribute.capitalizedName}.label)!'-'}</option>
																[/#list]
															</select>	
														</div>
													<#-- 文件类型 属性处理 -->
													<#elseif attribute.isAttributeTypeBeAnyFile() >	
														...
														<a href="${r'$'}{(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'})!}" class="btn btn-info btn-xs" target="_blank">${r'$'}{message("admin.common.view")}</a>
													<#-- N-1关联类型 属性处理(搜索属性) -->
													<#elseif attribute.isAttributeTypeBeN_1() && attribute.isN2OneTypeBeSearch() >
														<div class="clearfix">
															...
															&nbsp;&nbsp;
															<label class="selectedLabel control-label" [#if !(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'}??) ]style="display:none;"[/#if] >
																<span class="red" >当前选择：</span>
																<span class="value" >${r'$'}{(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'}.${(attribute.relatedDevEntity.titleAttribute)!'-'})!}</span>
															</label>
															&nbsp;&nbsp;
															<input type="hidden" style="width:50px!important;" id="${(attribute.unCapitalizedName)!'-'}Id" name="${(attribute.unCapitalizedName)!'-'}Id" value="${r'$'}{(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'}.id)!}" title="请填写关联实体的 【ID】属性" class="col-xs-5 col-sm-2" />
															<a href="javascript:void(0);" style="width:50px!important; [#if !(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'}??) ]display:none;[/#if]" class="clearInputs btn btn-info btn-xs" >清空</a>
														</div>
													<#-- N-1关联类型 属性处理(下拉列表) -->
													<#elseif attribute.isAttributeTypeBeN_1() && attribute.isN2OneTypeBeSelect() >
														<div class="clearfix">
															<select name="${(attribute.unCapitalizedName)!'-'}Id" class="col-xs-10 col-sm-5" >
																...
																[#list ${attribute.capitalizedName}s as ${attribute.capitalizedName}]
																<option value="${r'$'}{${attribute.capitalizedName}.id}" [#if (${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'}.id)! == ${attribute.capitalizedName}.id ]selected="selected"[/#if] >${r'$'}{(${attribute.capitalizedName}.${(attribute.relatedDevEntity.titleAttribute)!'-'})!'-'}</option>
																[/#list]
															</select>
														</div>
													...
													<#else>
														<div class="clearfix">
															<input name="${(attribute.unCapitalizedName)!'-'}" value="${r'$'}{(${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'})!}" type="text" class="col-xs-10 col-sm-5" />
														</div>
													</#if>
													</div><#-- end of col-sm-9 -->
			...
			$inputForm.validate({
				rules: {
					...
					<#if attribute.isUnique >
						remote: {
							url: "check_${(attribute.unHumpName)!'-'}_unique.jhtml?previous${attribute.capitalizedName}=${r'$'}{${(devEntity.unCapitalizedClassName)!'-'}.${(attribute.unCapitalizedName)!'-'}?url}",
							cache: false
						},
					</#if>
