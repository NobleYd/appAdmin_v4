...
(function($) {
	...
	$.dialog = function(options) {
		...
		if (settings.ok != null) {
			$dialogOk = $('<input type="button" class="btn btn-info btn-minier" value="' + settings.ok + '" \/>').appendTo($dialogBottom);
		}
		if (settings.cancel != null) {
			$dialogCancel = $('<input type="button" class="btn btn-info btn-minier" value="' + settings.cancel + '" \/>').appendTo($dialogBottom);
		}
		if (!window.XMLHttpRequest) {
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: app.base + "/admin/file/browser.jhtml",
				uploadUrl: app.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					if (settings.isUpload) {
						...
						$browserForm = $('<form class="pull-left" action="' + settings.uploadUrl + '" method="post" encType="multipart/form-data" target="' + browserFrameId + '"><input type="hidden" name="token" value="' + token + '" \/><input type="hidden" name="fileType" value="' + settings.type + '" \/><\/form>').appendTo($browserBar);
						$browserUploadButton = $('<a href="javascript:;" class="browserUploadButton btn btn-xs">' + message("admin.browser.upload") + '<\/a>').appendTo($browserForm);
						$browserUploadInput = $('<input type="file" name="file" \/>').appendTo($browserUploadButton);
					}
					$browserParentButton = $('<a href="javascript:;" class="parent btn btn-xs pull-left">' + message("admin.browser.parent") + '<\/a>').appendTo($browserBar);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = app.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = app.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
...
//一旦添加：为了解决有些情况token无法被自动添加的问题，此处写一个手动添加token的方法。
function addFormToken(form){
	var $form = $(form);
	if ($form.attr("method") != null && $form.attr("method").toLowerCase() == "post" && $form.find("input[name='token']").size() == 0) {
		var token = getCookie("token");
		if (token != null) {
